    ...
    <div v-if="!isLoading">
        ...
        <div class="admin-panel p-4">
            <div class="mb-4 flex items-center justify-between gap-3">
                <div>
                    <p class="m-0 text-sm text-[var(--admin-text-muted)]">{{ $t('customer.maintenanceRequest') }}</p>
            ...
            <div class="customer-detail-grid">
                ...
                <div class="customer-detail-item">
                    <p class="customer-detail-label">{{ $t('customer.category') }}</p>
                    <p class="customer-detail-value capitalize">{{ state.category || '—' }}</p>
                </div>
                <div class="customer-detail-item">
                    <p class="customer-detail-label">{{ $t('customer.priority') }}</p>
                    <p class="customer-detail-value capitalize">{{ state.priority || '—' }}</p>
                </div>
                <div class="customer-detail-item">
                    <p class="customer-detail-label">{{ $t('customer.submitted') }}</p>
                    <p class="customer-detail-value">{{ state.created_at || '—' }}</p>
                </div>
                <div class="customer-detail-item">
                    <p class="customer-detail-label">{{ $t('customer.updated') }}</p>
                    <p class="customer-detail-value">{{ state.updated_at || '—' }}</p>
                </div>
                <div class="customer-detail-item md:col-span-2">
                    <p class="customer-detail-label">{{ $t('customer.description') }}</p>
                ...
                <div v-if="state.rejection_reason" class="customer-detail-item md:col-span-2">
                    <p class="customer-detail-label">{{ $t('customer.rejectionReason') }}</p>
                ...
                <div v-if="state.resolution_note" class="customer-detail-item md:col-span-2">
                    <p class="customer-detail-label">{{ $t('customer.resolutionNote') }}</p>
