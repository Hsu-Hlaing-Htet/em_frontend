...
import { MAINTENANCE_CATEGORY_EXPORT_COLUMNS } from '@/helpers/lists/exportColumns';

export const useMaintenanceCategoryList = () => {
    const dt = ref();
    const search = ref('');
    ...
    const showConfirmDialog = (id) => {
        confirmDelete('Are you sure you want to delete this maintenance category?', async () => {
            ...
            EventBus.emit('show-toast', {
                severity: 'success',
                summary: '',
                detail: response?.message || 'Maintenance category deleted successfully.',
            });
    ...
    const loadingData = async () => {
        ...
        if (response) {
            const { data } = response;
            maintenanceCategories.value = data.data || [];
    ...
    const {
        ...
        filenameBase: 'maintenance-categories',
        columns: MAINTENANCE_CATEGORY_EXPORT_COLUMNS,
        emptyMessage: 'No maintenance categories available to export.',
        getFetchParams: () => ({
            order: multisortConvert(lazyParams.value.multiSortMeta),
            search: search.value,
        }),
