...
import { renderRentContractDocumentPage } from '@/helpers/documents/documentOutput';
...
export default function useShowActiveRent() {
    ...
    const cancelContract = async (payload) => {
        try {
            ...
            EventBus.emit('show-toast', {
                severity: 'success',
                summary: '',
                detail: store.getActionResponse?.message || 'Rent contract terminated successfully.',
            });
            await router.push(backRoute);
        } catch (error) {
            showApiErrorToast(error, 'Unable to terminate rent contract.');
        }
    };
    ...
    const sendEmail = async () => {
        ...
        try {
            const response = await rentService.sendDocumentEmail('active', state.id, {
                html: renderRentContractDocumentPage(currentDocument),
            });
