<template>
    <div class="admin-panel relative mx-auto max-w-6xl">
        <form
            class="grid grid-cols-1 gap-4 md:grid-cols-2"
            @submit.prevent="handleSubmit"
        >
            <div class="field">
                <label for="name" class="mb-2 block text-md">Name</label>
                <InputText
                    id="name"
                    v-model="state.name"
                    class="w-full"
                    placeholder="e.g. Standard Daily Fee"
                />
                <small v-if="errors.has('name')" class="p-error">
                    <div v-for="error in errors.get('name')" :key="error">{{ error }}</div>
                </small>
            </div>

            <div class="field">
                <label for="type" class="mb-2 block text-md">Type</label>
                <Dropdown
                    id="type"
                    v-model="state.type"
                    :options="typeOptions"
                    option-label="label"
                    option-value="value"
                    placeholder="Select type"
                    class="w-full"
                />
                <small v-if="errors.has('type')" class="p-error">
                    <div v-for="error in errors.get('type')" :key="error">{{ error }}</div>
                </small>
            </div>

            <div class="field">
                <label for="value" class="mb-2 block text-md">Value</label>
                <InputNumber
                    id="value"
                    v-model="state.value"
                    class="w-full"
                    :min="0"
                    :min-fraction-digits="2"
                />
                <small v-if="errors.has('value')" class="p-error">
                    <div v-for="error in errors.get('value')" :key="error">{{ error }}</div>
                </small>
            </div>

            <div class="field">
                <label for="per" class="mb-2 block text-md">Per</label>
                <Dropdown
                    id="per"
                    v-model="state.per"
                    :options="perOptions"
                    option-label="label"
                    option-value="value"
                    placeholder="Select per"
                    class="w-full"
                />
                <small v-if="errors.has('per')" class="p-error">
                    <div v-for="error in errors.get('per')" :key="error">{{ error }}</div>
                </small>
            </div>

            <div class="field">
                <label for="grace_days" class="mb-2 block text-md">Grace Days</label>
                <InputNumber
                    id="grace_days"
                    v-model="state.grace_days"
                    class="w-full"
                    :min="0"
                />
                <small v-if="errors.has('grace_days')" class="p-error">
                    <div v-for="error in errors.get('grace_days')" :key="error">{{ error }}</div>
                </small>
            </div>

            <div class="field">
                <label for="status" class="mb-2 block text-md">Status</label>
                <Dropdown
                    id="status"
                    v-model="state.status"
                    :options="statusOptions"
                    option-label="label"
                    option-value="value"
                    placeholder="Select status"
                    class="w-full"
                />
                <small v-if="errors.has('status')" class="p-error">
                    <div v-for="error in errors.get('status')" :key="error">{{ error }}</div>
                </small>
            </div>

            <div class="flex justify-end gap-2 md:col-span-2">
                <Button type="submit" label="Save" />
                <router-link :to="{ name: 'lateFeeList' }">
                    <Button type="button" label="Cancel" />
                </router-link>
            </div>
        </form>
    </div>

    <Loading v-if="isLoading" />
</template>

<script>
import { defineComponent } from 'vue';
import Dropdown from 'primevue/dropdown';
import InputText from 'primevue/inputtext';
import InputNumber from 'primevue/inputnumber';
import Button from 'primevue/button';
import Loading from '@/components/global/Loading.vue';
import useNewLateFee from './useNewLateFee';

export default defineComponent({
    name: 'NewLateFee',
    components: { Dropdown, InputText, InputNumber, Button, Loading },
    setup() {
        return useNewLateFee();
    },
});
</script>
